...
// ESM-friendly. Generates synthetic HogFlix engagement for PostHog.
// Modes:
...
//
// Env (common):
//   PH_PROJECT_API_KEY, PH_HOST
//
// Env (backfill):
//   BACKFILL_WEEKS=8          // how many whole weeks to backfill (0 = off)
//   USERS_PER_WEEK=40         // seed users per week
//   WEEKLY_GROWTH_PCT=3       // +% growth per newer week (e.g., newer weeks have more users)
//   RETURN_RATE=0.45          // % of users who also "return" the next week
//
// Env (live):
//   BASE_SESSIONS=28          // baseline sessions per run
//   WEEKLY_GROWTH_PCT=2       // gentle growth over time
//   WEEKEND_DAMPEN=0.65       // weekend volume multiplier (0..1)
//   EOM_DROOP_PCT=15          // dip when in last 5 days of month
//   NOISE_PCT=25              // +/- random variation each run (%)
...
// Companies & plan tiers as requested
const COMPANIES = [
...
function daysAgo(n) {
  ...
  d.setUTCHours(11, 0, 0, 0) // anchor around midday UTC to avoid TZ edges
  return d
}
function weeksAgo(n) {
  const d = startOfDayUTC()
...
function weeklyGrowthFactor(d, pct = WEEKLY_GROWTH_PCT) {
  // Using weeks since a fixed baseline
...
function jitter(base, pct = NOISE_PCT) {
  const delta = (Math.random()*2 - 1) * (pct/100) // +/-pct%
...
async function identifyOnce(distinctId, personProps, ts) {
  await client.capture({
    ...
    properties: {
      $set: personProps,
      $set_once: { first_seen: ts.toISOString() },
    },
    timestamp: ts,
  })
}
...
async function simulateSessionAt(distinctId, profile, ts) {
  const { persona, device, company, utm } = profile

  // pageviews across basic routes
  ...
  // Optional plan selection (~25%)
  if (Math.random() < 0.25) {
    await client.capture({
      distinctId,
      event: 'plan_selected',
      properties: { plan: company.plan, is_synthetic: true },
      timestamp: ts,
    })
    await client.capture({
      distinctId,
      event: '$set',
      properties: { $set: { company_plan: company.plan } },
      timestamp: ts,
    })
  }
}
...
async function runBackfill() {
  // Newer weeks (closer to now) get more users via WEEKLY_GROWTH_PCT
  for (let w = BACKFILL_WEEKS; w >= 0; w--) {
    ...
    for (let i = 0; i < usersThisWeek; i++) {
      ...
      // initial visit day inside that week (0..6)
      const dayOffset = Math.floor(Math.random() * 7)
      const ts = new Date(weekStart.getTime() + dayOffset * 24*3600*1000)
      ...
      if (w > 0 && Math.random() < RETURN_RATE) {
        const retStart = weeksAgo(w - 1)
        const retOffset = Math.floor(Math.random() * 7)
        const ts2 = new Date(retStart.getTime() + retOffset * 24*3600*1000)
